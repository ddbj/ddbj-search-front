...
- `src/api/detail/<dbType>.ts`
- `src/api/detail/base.ts`
...
1. 比較基準を確認する
   - 対象DBTypeに対応する `src/api/detail/<dbType>.ts` を読む。
   - `baseDetailResponseSchema` を継承している場合は `src/api/detail/base.ts` も読む。
