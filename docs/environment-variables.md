...
| 名前            | 用途                            | 既定値                                  | 設定例                                                     | 備考                             |
| --------------- | ------------------------------- | --------------------------------------- | ---------------------------------------------------------- | -------------------------------- |
| `VITE_API_PATH` | MSWを使わないときのAPI base URL | `https://staging.nig.ac.jp/search/api/` | `VITE_API_PATH=https://ddbj-staging.nig.ac.jp/search/api/` | 未設定時はコード上の既定値を使う |
| `VITE_MSW`      | Mock Service Workerを有効にする | 未設定                                  | `VITE_MSW=true`                                            | `true` のときだけMSWを有効にする |
